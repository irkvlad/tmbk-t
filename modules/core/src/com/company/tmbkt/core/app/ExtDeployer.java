...
@ManagedBean("tmbk-t_ExtDeployer")
public class ExtDeployer extends AbstractDeployer implements ExtDeployerMBean {
    ...
    protected void checkForFirstInit() {
        ...
        try {
            ...
            List<Config> configs = em.createQuery("select c from sys$Config c where c.name='tmbk-t.initDefaultInFirstStart'",
                ...
                if (StringUtils.isNotBlank(edition) && Integer.parseInt(edition) > 1) {
                    ...
                    if (initDefaultInFirstStart == null) {
                        try {
                            ...
                            config.setName("tmbk-t.initDefaultInFirstStart");
                        ...
                        try {
                            login();
                            em.createQuery("update sys$Config c set c.value='false' where c.name='tmbk-t.initDefaultInFirstStart'",
    ...
    protected void executeInitScripts() {
        ...
        File folderDB = new File(dbDirPath + "/50-tmbk-t/init/" + dbmsType);
